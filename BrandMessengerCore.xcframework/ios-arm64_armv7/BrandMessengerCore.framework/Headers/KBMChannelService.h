...
/// @param completion if an error is nil then the group is created successfully it has KBMChannel information of channel else some error while creating if an error is not nil.
- (void)createChannel:(NSString *)channelName
   orClientChannelKey:(NSString * _Nullable)clientChannelKey
       andMembersList:(NSMutableArray *)memberArray
         andImageLink:(NSString * _Nullable)imageLink
       withCompletion:(void(^)(KBMChannel * _Nullable channel, NSError * _Nullable error))completion DEPRECATED_MSG_ATTRIBUTE("Use createChannelWithChannelInfo:withCompletion instead");
...
/// Types of the group. PRIVATE = 1,PUBLIC = 2, OPEN = 6
/// @param metaData It's extra information can be added in channel.
/// @param completion if error is nil then group is created successfully it has KBMChannel infomration of channel else some error while creating if error is not nil.
- (void)createChannel:(NSString *)channelName
   orClientChannelKey:(NSString * _Nullable)clientChannelKey
       andMembersList:(NSMutableArray *)memberArray
         andImageLink:(NSString * _Nullable)imageLink
...
/// Types of the group. PRIVATE = 1,PUBLIC = 2, OPEN = 6
/// @param metaData It's extra information can be added in channel.
/// @param completion If an error is nil then a group is created successfully it has KBMChannel information of channel else some error while creating if an error is not nil.
- (void)createChannel:(NSString *)channelName
...
/// @param metaData It's extra information can be added in channel.
/// @param adminUserId If you want to make any member as admin while creating then you can pass the userId of that member.
/// @param completion if an error is a nil then a group is created successfully it has KBMChannel information of channel else some error while creating if an error is not nil.
- (void)createChannel:(NSString *)channelName
   orClientChannelKey:(NSString * _Nullable)clientChannelKey
       andMembersList:(NSMutableArray *)memberArray
         andImageLink:(NSString * _Nullable)imageLink
...
/// @param channelKey Pass the channelkey or groupId that is required to get the channel information.
/// @param clientChannelKey Pass the clientChannelKey in case if the channelkey is not present else it will be nil.
/// @param completion On successful fetch it will have KBMChannel object else on an error in fetching it will be nil.
- (void)getChannelInformation:(NSNumber * _Nullable)channelKey
           orClientChannelKey:(NSString * _Nullable)clientChannelKey
               withCompletion:(void (^)(KBMChannel * _Nullable channel)) completion DEPRECATED_MSG_ATTRIBUTE("Use getChannelInformationByResponse:orClientChannelKey:withCompletion instead");

/// Returns Channel user for given channel key.
- (KBMChannelUserX * _Nullable)loadChannelUserX:(NSNumber *)channelKey DEPRECATED_ATTRIBUTE;
